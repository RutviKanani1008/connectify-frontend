...
const Register = () => {
  ...
  const [formData, setFormData] = useState({ company: null, admin: null });
  ...
  const onSubmit = async () => {
    ...
    rawData.company.grandLodgeId = rawData?.company?.grandLodgeId?.value
      ? rawData?.company?.grandLodgeId?.value
      : null;
    if (rawData.company.role === 'admin') {
      rawData.company.isGrandLodge = false;
    } else {
      rawData.company.isGrandLodge = true;
    }
    rawData.admin.role = rawData.company.role;
    // rawData.token = recaptchaToken;

    const res = await register(rawData);
    if (!res.error) {
      showToast(TOASTTYPES.success, '', 'Successfully registered');
                ...
                <>
                  <div className='thank-you-wrapper'>
                    ...
                    <p className='normal-text'>
                      ...
                      , Please verify your email from the link we have sent you
                      in the email. Once your email is verified, we will review
                      your request. For any questions please contact us at
                      support@xyz.com.
                    </p>
