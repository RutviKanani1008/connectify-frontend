...
// ====================================================
import InvoiceActionItem from './InvoiceActionItem';
...
export const InvoiceAction = ({
  ...
  // ========================== Hooks ================================
  const invoiceStatusActions = useWatch({
    control,
    name: `invoiceStatusActions`,
  });
  ...
  return (
    <div>
      ...
      {enableStatusAction && (
        <Row className='mt-1'>
          {currentCustomer
            ? invoiceStatusActions?.length > 0 &&
              invoiceStatusActions?.map((_, idx) => {
                return (
                  <InvoiceActionItem
                    key={idx}
                    watch={watch}
