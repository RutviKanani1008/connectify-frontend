...
import { useUpdateEmailSenderAPI } from '../Admin/campaigns/mass-email-tool/service/emailSender.service';
...
const VerifyEmail = () => {
  ...
  // API Service
  const { isLoading, updateEmailSenderAPI } = useUpdateEmailSenderAPI();
  useEffect(() => {
    updateEmailSender();
  }, []);

  const updateEmailSender = async () => {
    await updateEmailSenderAPI({ email: query.email, company: query.company });
  };
