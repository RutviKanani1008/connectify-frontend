  ...
  const toggleOpenGroup = (item, parent) => {
    ...
    if (openGroup.includes(item.id)) {
      ...
      if (
        ...
        // Do not close active group if menu is collapsed, to open the menu and sub item lists
        if (!menuCollapsed) activeGroup.splice(activeGroup.indexOf(item.id), 1);
      }

      // Open Menu to show sub items
  ...
  return (
    <li
      className={classnames('nav-item has-sub', {
        ...
        // Hide sidebar parent background colors
