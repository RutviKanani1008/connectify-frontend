...
import {
  ...
  useGetFeatureRequestById,
  useGetFeatureRequests,
  useReadNewFeatureRequests,
...
import { useFeatureReqColumns } from './hooks/useFeatureRequest';
import AddFeatureRequest from '../../settings/AddFeatureRequest';
...
import { featureRequestStatus } from '../../../constant';
...
const FeatureRequests = () => {
  ...
  const [openAddNewRequest, setOpenAddNewRequest] = useState(null);
  const [openComments, setOpenComments] = useState(null);

  const [featureData, setFeatureData] = useState({ results: [], total: 0 });

  const [openPreview, setOpenPreview] = useState(null);
  const [openPreviewImage, setOpenPreviewImage] = useState(null);

  const { getFeatureRequests, isLoading } = useGetFeatureRequests();
  const { updateFeatureRequest, isLoading: updateLoader } =
    useUpdateFeatureRequest();
  const { readNewFeatureRequests } = useReadNewFeatureRequests();
  const { deleteFeatureRequest } = useDeleteFeatureRequest();
  const { getFeatureRequestById } = useGetFeatureRequestById();

  const { columns } = useFeatureReqColumns({
    handleDelete,
  ...
  useEffect(() => {
    if (queryParams.has('id')) {
      const select =
        'id,firstName,lastName,email,phone,requestMessage,fileUpload,status,comments,isNew,userId,lastReadedCommentId,createdAt';

      getFeatureRequestById(queryParams.get('id'), { select })
        .then((res) => {
    ...
    dispatch(setSidebarCount({ featureRequest: 0 }));
  }, []);

  useEffect(() => {
    if (activeView === 'grid') {
      setCurrPage(1);
      loadAllFeatureRequests({ page: 1 }, true);
    }
  }, [activeView]);

  /* Read New Tickets */
  useEffect(() => {
    const newFeatureRequestsIds = featureData.results
      .filter((r) => r.isNew)
      .map((r) => r._id);

    readAllNewFeatureRequests(newFeatureRequestsIds);
  }, [featureData.results]);
  ...
  const readAllNewFeatureRequests = async (ids = []) => {
    if (isSuperAdmin && ids.length) {
      await readNewFeatureRequests({ featureRequestIds: ids });
    }
  };

  const readAllNewComments = async (openComments) => {
    const featureRequestId = openComments._id;
  ...
  const loadAllFeatureRequests = async (params = {}, reset = false) => {
    const {
      search = searchVal,
      page = currPage,
    ...
    const { data, error } = await getFeatureRequests(reqParams);
    if (!error) {
      const { featureRequests, total } = data;

      if (_.isArray(featureRequests)) {
        const newRecords =
          reset || activeView === 'list'
            ? featureRequests
            : [...(featureData.results || []), ...featureRequests];

        setFeatureData({ results: newRecords, total });
      }
    }
  };
  ...
  const debounceFn = useCallback(
    _.debounce(async (search) => {
      setCurrPage(1);
      await loadAllFeatureRequests({ search, page: 1 }, true);
    }, 300),
    []
  );
  ...
  async function handleDelete(id) {
    const result = await showWarnAlert({
      title: 'Are you sure?',
      text: 'Are you sure you would like to delete this Feature Request?',
    });

    if (result.value) {
      const { error } = await deleteFeatureRequest(
        id,
      ...
      if (!error) {
        if (activeView === 'grid') {
          setCurrPage(1);
          await loadAllFeatureRequests({ page: 1 }, true);
        } else {
  ...
  const handleStatusUpdate = async (item, newStatus) => {
    ...
    if (status !== item?.status) {
      ...
      if (result.value) {
        const { error } = await updateFeatureRequest(item._id, { status });
        if (!error) {
          setFeatureData((prev) => ({
            ...prev,
  ...
  const handleCloseAddFeatureRequest = () => {
    setOpenAddNewRequest(false);
  };

  const ticketStatus = (status) => {
    return featureRequestStatus.find((s) => s.value === status);
  };
  ...
  return (
    <>
      <div className='feature-requests-page'>
        <h2 className='mobile-page-title'>Feature Requests</h2>
        <div className='feature-requests-header d-flex justify-content-between align-items-center'>
          <Button
            className='add-btn'
            color='primary'
            onClick={() => {
              setOpenAddNewRequest(true);
            }}
          >
            <Plus size={15} />
            <span className='btn-text'>Add Feature Request</span>
          </Button>
        ...
        {activeView === 'grid' ? (
          <UILoader blocking={isLoading}>
            <Card className='support__tickets__card feature-requests-card'>
              <CardHeader>
                <CardTitle>
                  <h4 className='card-title'>Feature Requests</h4>
                </CardTitle>
 ...
 C39.6,31.5,32.4,38.7,23.5,38.7z'
              ...
              <CardBody
                className={`pb-0 fancy-scrollbar ${
                  featureData.results.length < featureData.total &&
                  'load-more-active'
                }`}
              >
                <Row className='report-problem-row'>
                  {featureData.results?.length > 0 ? (
                    featureData.results.map((item, index) => {
                      return (
                        <Fragment key={index}>
                          <Col
                    ...
                    <div className='d-flex justify-content-center m-4'>
                      <span className='no-data-found'>
                        {!isLoading && 'No Feature Requests found!'}
                      </span>
                    </div>
                  )}
                </Row>
                ...
                <div className='text-center loadMore-btn-wrapper'>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      loadAllFeatureRequests({ page: currPage + 1 });
                    }}
                  >
                    Load More
                  </Button>
          ...
          <>
            <div className='table-view'>
              <ServerSideTable
                ...
                columns={columns}
                getRecord={loadAllFeatureRequests}
                data={featureData}
                itemsPerPage={10}
              />
            </div>
        ...
        {openAddNewRequest && (
          <>
            <AddFeatureRequest
              open={openAddNewRequest}
              onClose={handleCloseAddFeatureRequest}
              onNewAdded={() => {
                setCurrPage(1);
                loadAllFeatureRequests({ page: 1 }, true);
              }}
      ...
      <Modal
        scrollable={true}
        isOpen={!!openPreview}
        className={`modal-dialog-centered feature-request-preview-modal modal-dialog-mobile`}
          ...
          {openPreview && (
            <Fragment>
              <div style={{ display: 'none' }}>
                ...
                <div className='support-ticket-preview-row'>
                  <div className='date-label'>Message</div>
                  <div className='data-value'>{openPreview.requestMessage}</div>
                </div>
                {openPreview.fileUpload && (
                  <div className='d-flex item'>
                    <h5 className='me-2 date-label'>Attachment</h5>
                      ...
                      <div className='file__card m-1'>
                        <div className='d-flex justify-content-center file__preview__wp'>
                          <div className='file__preview__sm'>
                            <img
                              src={`${process.env.REACT_APP_S3_BUCKET_BASE_URL}${openPreview.fileUpload}`}
                              alt='preview-doc'
                            />
                          </div>
                          <div
                            ...
                            onClick={() => {
                              setOpenPreviewImage(openPreview.fileUpload);
                            }}
                          >
                            <Eye color='#ffffff' />
                          </div>
